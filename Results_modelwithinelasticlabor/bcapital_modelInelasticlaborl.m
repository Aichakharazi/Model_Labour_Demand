function [fx,fxp,fy,fyp,fypyp,fypy,fypxp,fypx,fyyp,fyy,fyxp,fyx,fxpyp,fxpy,fxpxp,fxpx,fxyp,fxy,fxxp,fxx,f] = bcapital_modelInelasticlaborl
...
f13 = -bbeta/coldV  + bbeta/coldVV *bbeta * (1+rsV)   ; %   bbeta/coldV - lambdacV;
 ...
 % F.O.C. wrt to lm 
f14 = -LMV + ((lambdacV * e_mV * omegam)/(bbeta*y_piV) )^(1/eeta); 

f15 = -l_hftmV + ((lambdacV * e_mV * om_hf_init )/(bbeta*y_piV) )^(1/eeta); 
f16 = -l_hntmV + ((lambdacV * e_mV * om_hn_init )/(bbeta*y_piV) )^(1/eeta); 
f17 = -l_lftmV + ((lambdacV * e_mV * om_lf_init )/(bbeta*y_piV) )^(1/eeta); 
f18 = -l_lntmV + ((lambdacV * e_mV * om_ln_init )/(bbeta*y_piV) )^(1/eeta); 


 % F.O.C. wrt to ly


f19 = -LYV + ((lambdacV * e_mV * omegay)/(bbeta*y_piV) )^(1/eeta) ;  %  

f20 = -l_hftyV  + ((lambdacV * e_mV * oy_hf_init)/(bbeta*y_piV) )^(1/eeta) ;  %  
f21 = -l_hntyV  + ((lambdacV * e_mV * oy_hn_init)/(bbeta*y_piV) )^(1/eeta) ;  %  
f22 = -l_lftyV  + ((lambdacV * e_mV * oy_lf_init)/(bbeta*y_piV) )^(1/eeta) ;  %  
f23 = -l_lntyV  + ((lambdacV * e_mV * oy_ln_init)/(bbeta*y_piV) )^(1/eeta) ;  %  
...
xp = [cyVV cmVV coldVV   lambdacVV   zzVV   ];
yp = [ kkVV ...
